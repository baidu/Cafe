...
public class AndroidWebDriver implements WebDriver, SearchContext, JavascriptExecutor,
  ...
  public AndroidWebDriver(Activity activity,WebView wv){
	  this.activity = activity;
...
//	    // Needs to be called before CookieMAnager::getInstance()
	    ...
	    //using pre-set webview, support android native app's webview
	    webview=wv;
	    WebDriverWebView.initWebView(this, webview);
  ...
  //add by lyj: wait for webview loaded
  public void waitForWebViewLoadCompleted(){
	  navigation.waitForPageLoadToComplete();
  }
