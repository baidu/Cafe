...
public class LocalLib extends SoloEx {
	 ...
	 * @throws IllegalArgumentException
	 */
	public static ArrayList<String> getPropertyNameByValue(Object owner, int classLevel, Class<?> valueType,
	...
	public Object getListener(View view, Class<?> targetClass, String fieldName) {
		...
		try {
			if (!(view instanceof AdapterView) && Build.VERSION.SDK_INT > 14) {// API
				...
				// Android
				// 4.0
				Object mListenerInfo = ReflectHelper.getObjectProperty(view, level, "mListenerInfo");
				return null == mListenerInfo ? null : ReflectHelper.getObjectProperty(mListenerInfo, 0, fieldName);
			} else {
	...
	private int getRStringId(String packageName, String stringName) {
		...
		try {
			return (Integer) stringClass.getDeclaredField(stringName).get(stringClass.newInstance());
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		} catch (SecurityException e) {
			e.printStackTrace();
	...
	public static View getRecentDecorView(View[] views) {
		return (View) invoke(mViewFetcher, "getRecentDecorView", new Class[] { View[].class }, new Object[] { views });
	}
	...
	public String getAppNameByPID(int pid) {
		ActivityManager manager = (ActivityManager) mInstrumentation.getTargetContext().getSystemService(
	...
	public <T extends View> ArrayList<T> getViews(Class<T> classToFilterBy, boolean onlySufficientlyVisible) {
		ArrayList<T> targetViews = new ArrayList<T>();
		ArrayList<View> views = (ArrayList<View>) invoke(mViewFetcher, "getViews", new Class[] { View.class,
				boolean.class }, new Object[] { null, onlySufficientlyVisible });// viewFetcher.getViews(null,
		// false);
		for (View view : views) {
	...
	public View getViewByFamilyString(String familyString) {
		...
		while (System.currentTimeMillis() < endTime) {
			...
			for (View decorView : decorViews) {
				ArrayList<View> invisibleViews = removeInvisibleViews(getCurrentViews(View.class, decorView));
	...
	public View waitForViewByFamilyString(String familyString, String className) {
		...
		String assertString = String.format("Except view [%s], Actual view [%s]", className, exceptViewClassName);
	...
	public void waitForTextByFamilyString(String familyString, String text) {
		...
		Assert.assertTrue(String.format("Except text [%s], Actual text [%s]", text, actual), actual.equals(text));
	}
	...
	public boolean isInScreen(View view) {
		...
		return location[0] < 0 || location[0] > getDisplayX() || location[1] < 0 || location[1] > getDisplayY() ? false
				: true;
	}
	...
	public void scrollListToLineWithFamilyString(final int line, final String familyString) {
		...
		invoke(mScroller, "scrollListToLine", new Class[] { AbsListView.class, int.class }, new Object[] { absListView,
				line });
	}

	public void scrollScrollViewToWithFamilyString(final String familyString, final int x, final int y) {
	...
	public boolean waitForView(String className, final int index, final int timeout, final boolean scroll) {
		boolean ret = false;
		try {
			Class<?> viewClass = Class.forName(className);
			ret = (Boolean) invoke(mWaiter, "waitForView", new Class[] { Class.class, int.class, int.class,
	...
	public String getFamilyString(View v) {
		...
		while (view.getParent() instanceof ViewGroup) {
			...
			if (Build.VERSION.SDK_INT >= 14 && parent.getClass().getName().equals(CLASSNAME_DECORVIEW)) {
			} else {
	 ...
	 */
	public WebElement waitForWebElementByFamilyString(String familyString, int timeout, boolean scroll) {
		...
		while (true) {
			...
			if (timedOut) {
				...
				invoke(mSearcher, "logMatchesFound", new Class[] { String.class }, new Object[] { familyString });
				return null;
			}
			...
			boolean javaScriptWasExecuted = (Boolean) invoke(mWebUtils, "executeJavaScriptFunction",
			...
			try {
				List<WebElement> webElements = (List<WebElement>) ReflectHelper.getObjectProperty(mSearcher, 0,
						"webElements");
				// searcher.addViewsToList(webElements, viewsFromScreen);
				invoke(mSearcher, "addViewsToList", new Class[] { List.class, List.class }, new Object[] { webElements,
						viewsFromScreen });

				// searcher.getViewFromList(webElements, 1);
				WebElement webElementToReturn = (WebElement) invoke(mSearcher, "getViewFromList", new Class[] {
				...
				if (scroll) {
					...
					invoke(mScroller, "scroll", new Class[] { int.class }, new Object[] { getField(mScroller, "DOWN") });
				}
			} catch (SecurityException e) {
				e.printStackTrace();
			} catch (IllegalArgumentException e) {
	 ...
	 * 
	 * @param familyString the String object, used to locates an identified element
	...
	public void enterTextInWebElementByFamilyString(String familyString, String text) {
		...
		// executeJavaScriptFunction(js);
		invoke(mWebUtils, "executeJavaScriptFunction", new Class[] { String.class }, new Object[] { js });
	}
	...
	public void dumpPage() {
		...
		for (WebElement element : elements) {
			print("(" + element.getLocationX() + "," + element.getLocationY() + ") , {tagName : "
					...
					+ element.getClassName() + "} , {name : " + element.getName() + "} , {text : " + element.getText()
					+ "}");
		}
		sleep(5000);
		print("############# dumpPage end #################");
